...
classdef GlobalStiffnessMatrixComputer < handle
    ...
    properties (Access = private)
        Tnodal %guardar els inputs

    end

    methods (Access = public)

        function obj = GlobalStiffnessMatrixComputer(Kel,Tnodal,nnod, ndof) %Que el codi no s'allargui, fer struct --> (cParams) INPUT CONSTRUCTOR
            %obj = obj.init(cParams) --> Només va això al constructor
            %(normalment)
        end

        function K_g = compute(obj)
            K_g = zeros(nDof*nNod,nDof*nNod);
            nElm = size (obj.Tnodal,1);
            for e = 1:(nElm)
                for i = 1:(2*ndof)
                    for j = 1:(2*ndof)
                        dofs_node_a = obj.Tnodal(e,i);
                        dofs_node_b = obj.Tnodal(e,j);
                        K_g(dofs_node_a,dofs_node_b) = K_g(dofs_node_a,dofs_node_b) + Kel(i,j,e); %si s'allarga molt, fer les operacions per parts i guardar el resultat en var.
                    end



%gDOFnodeA = obj.nDOFnode(obj.Tnod(e,1)-1)+(1:obj.nDOFnode);
%                gDOFnodeB = obj.nDOFnode(obj.Tnod(e,2)-1)+(1:obj.nDOFnode); 
 %               DOF = [gDOFnodeA, gDOFnodeB];
  %              K(DOF,DOF)=K(DOF,DOF)+obj.Kel(:,:,e);

  %fer loops sobre elements no es rentable


                end
            end
            %La F s'ha de fer en un altre objecte
        end
    end

    methods (Access = private)
        function obj = init (obj,cParams) %Fer sempre!
            obj.Tnodal = cParams.Tnodal;
        end

    end
end
%juntar aixo amb lo de la setmana vinent --> fer un parell de testos per
%aquestes classe!!!
%%
